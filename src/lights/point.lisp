...
(defun-g shadow-factor ((light-sampler :sampler-cube-array)
  ...
  (let* ((frag-to-light (- frag-pos light-pos))
         ...
         (grid-samples  (vector (v! 1 1  1) (v!  1 -1  1) (v! -1 -1  1) (v! -1 1  1)
    ...
    (dotimes (i 20)
      (let* ((closest-depth
               (x (texture light-sampler
                           (v! (+ frag-to-light (* disk-radius (aref grid-samples i))) index))))
             (closest-depth (* far-plane closest-depth)))
        (when (> (- current-depth bias)
                 closest-depth)
          (incf shadow 1f0))))
    (- 1 (divf shadow 20f0))))
