...
(defun-g textured-forward-cg-ibl-frag ((uv        :vec2)
                                       (frag-norm :vec3)
                                       (frag-pos  :vec3)
                                       (tbn       :mat3)
                                       (dir-pos   (:vec4 2))
...
(defmethod paint ((scene scene-ibl) (camera forward) (actor textured-ambient-cg) time)
  (with-slots (buf material scale uv-repeat dispscale
               albedo normal aomap roughmap dispmap)
      actor
