...
(defun-g point-light-apply ((color       :vec3)
                            ...
                            (linear      :float)
                            (quadratic   :float))
  (point-light-apply color light-color light-pos frag-pos normal 1f0 linear quadratic))
...
;; Only lambert diffuse, no spec, with ambient from light-color
...
;; Only oren diffuse, with ambient from light-color
(defun-g dir-light-apply ((color       :vec3)
                          (light-color :vec3)
                          (light-pos   :vec3)
                          (frag-pos    :vec3)
...
;; Lambert diffuse + specular, with ambient from light-color
...
(defun-g dir-light-strength ((color       :vec3)
                             (light-color :vec3)
                             (light-pos   :vec3)
                             (frag-pos    :vec3)
                             (normal      :vec3))
  "dir, no spec, no ambient, useful for vxgi"
  (let* ((light-dir (normalize (- light-pos frag-pos)))
         ;; Diffuse shading
         (diff    (saturate (dot normal light-dir)))
         ;; combine
