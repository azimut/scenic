...
(def-simple-main-loop play-render (:on-start #'init)
  (let* ((scene  (current-scene))
    ...
    (as-frame
      (blit (post scene) camera dt)
      ;;(draw-tex-br (first (sam camera)))
      ;; (draw-tex-bl (second (sam camera)))
      ;; (draw-tex-tr (third (sam camera)))
      ;; (draw-tex-tl (fourth (sam camera)))
      ;; (draw-tex-tr (first (sam (capture    (current-scene)))))
      ;; (draw-tex-tl (first (sam (prefilter  (current-scene)))))
      ;; (draw-tex-bl (first (sam (irradiance (current-scene)))))
